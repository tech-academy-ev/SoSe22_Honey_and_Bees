...
On a later stage of the project we would like to merge it with the honey data set. To this end, we will now spend some time preparing the bees data.
...
::: {.tips .r data-latex="r"}
You can use the lubridate package to transform "years" into a date format.
:::
...
::: {.tips .r data-latex="r"}
Have you heard of "sapply"? This might help you.
:::
...
We want to merge the honey and bees data set to analyze them jointly. To do so we need to reduce the frequency of the bees data frame (quarterly) to yearly.
...
::: {.tips .r data-latex="r"}
The key commands that you can use for this within ggplot() are geom_bar() and coord_flip(). Check back on the ggplot2 cheat sheet if you are feeling unsure! 
:::
...
Next, let's visualize the data with beautful Boxplots. Within your Team generate 4 Boxplot  aggregated for each year. Therefore, each Team member will present one boxplot, Choose from: Honey Production, Parasite: Varroa Mite, Precipitation, Bee Colonies. Tip: It will be 6 boxes since we look at 6 years.
...
```
```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.show="hold", out.width="50%"}
knitr::include_graphics("plot/Bee Colonies.jpg")
...
The idea is to combine the geographic information on the states with the numeric data to provide a rich visual that reflects the two combinations.

```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.align="center", out.width = "100%"}
knitr::include_graphics("plot/States_map_bee_starting_pop_popups.gif")
```
