...
Create a new column where you add the percentage of lost colonies in reference to the starting colonies to your data frame (it is as simple as it reads). 


### Group Means

We want to merge the honey and bees data set to analyze them jointly. To do so we need to reduce the frequency of the bees data frame (quarterly) to yearly, so data for 2015, 2016, 2017, 2018, 2019, and 2020 per state is left.
...
Next, let's visualize the split data with beautiful Boxplots. Generate 4 Boxplot aggregated for each year.  They should look at Honey Production, Parasite: Varroa Mite, Precipitation, and Bee Colonies. 
...
Here is the right place to put down anything you did and would like to do with the project that did not quite fit in the other exercise sections. If you want to get inspired by various visualization methods check out [this page](https://r-graph-gallery.com/).
...
For a TechAcademy certificate as a beginner you should complete 2/3 of the above. If you are advanced all of the above should be included in your output.
