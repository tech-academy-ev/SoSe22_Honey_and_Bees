...
### Task 11: Bar Plot for all States (starting bee colonies = number of bee poplation, sorted). What state has the highest colonies population?
...
#Isi: bekomme nicht hin, das es richitg abwecheselnd schwarz gelb ist 


###Task 12: Split a subset for data between the X different Team members (we have 40 States) (ungraded)
#HB$Group <- c(rep(1,60),rep(2,60), rep(3,60), rep(4, 60))

###Task 13: Boxplot: auf Jahr: Honigproduktion, Diseases, Regen , Bienencolonien (6 boxen)

HB1 <- HB %>% 
  filter(Group == 1) %>% 
  select(Year, State, production_kg) 


#heatMAP
ggplot(HB1, aes(y= State, x= Year)) +
  geom_tile(aes(fill = production_kg))+ 
...
Heat Map gefärbt je nach  ! GLEICHES SCALINGGGGGG!
*wie viel Honig produziert wird, 
*wo die meisten Bienenkolonien sind, 
*höchste Anzahl von Bienenkrankheiten (welche genau) , 
*höchstes Bienensterben (4 Teammitglieder = 4 Verschiedene Mappen) 
...
##ADVANCED: NICHT HONEY PRODUCTION, quarterly data für advanced
# paper: Bienen Krankheiten ~ Wetter , quarterly data 
# Wetter daten zu quarterly
# Regeression:    , nicht nur linear? quadtratisch?
# correlation plot
# regression erweitern, und kommentieren